...
export default function Age() {
  ...
  // Load saved age from local storage on mount
  useEffect(() => {
    const loadAge = async () => {
      try {
        const storedAge = await AsyncStorage.getItem("age");
        if (storedAge) {
          const ageNumber = parseInt(storedAge, 10);
          setSelectedAge(ageNumber);

          // Scroll to the saved age
          const index = AGES.indexOf(ageNumber);
          if (index >= 0) {
        ...
        console.log("Failed to load age from storage", e);
      }
    };
    loadAge();
  }, []);
  ...
  const saveAndContinue = async () => {
    try {
      ...
      console.log("Failed to save age", e);
    }
  };
  ...
  const renderAgeItem = ({ item, index }) => {
    ...
    return (
      ...
      >
        <Text style={[styles.ageText, { color: theme.text }]}>{item}</Text>
      </Animated.View>
    );
  };
  ...
  return (
    <View style={[styles.overlay, { backgroundColor: theme.background }]}>
      <Text style={[styles.quote, { color: theme.text }]}>SELECT YOUR AGE</Text>
      ...
      <View style={styles.spinnerContainer}>
        ...
        <View style={[styles.agesListContainer, { backgroundColor: theme.chart }]}>
          <Animated.FlatList
            ref={flatListRef}
            data={AGES}
...
const styles = StyleSheet.create({
  ...
  spinnerContainer: { height: 120, width: "110%", position: "relative", marginBottom: 40 },
  ...
  agesListContainer: { flex: 1, justifyContent: "center", width: "100%" },
  flatListContent: { alignItems: "center", paddingHorizontal: screenWidth / 2 - 35 },
  ageItem: { width: 70, height: 80, justifyContent: "center", alignItems: "center" },
  ageText: { fontSize: 24, fontWeight: "800", textAlign: "center" },
  selectedAgeContainer: { marginBottom: 40, alignItems: "center" },
  selectedAgeText: { fontSize: 18, fontWeight: "600", textAlign: "center" },
